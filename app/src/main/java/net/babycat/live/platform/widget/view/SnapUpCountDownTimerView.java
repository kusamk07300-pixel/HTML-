...
public class SnapUpCountDownTimerView extends LinearLayout {
    ...
    public SnapUpCountDownTimerView(Context context) {
        super(context);
        this.handler = new Handler() { // from class: net.babycat.live.platform.widget.view.SnapUpCountDownTimerView.1
    ...
    private void initView() {
        LayoutInflater.from(this.mContext).inflate(R.layout.view_countdowntimer, this);
    ...
    public void start() {
        if (this.timer == null) {
            ...
            this.timer = timer;
            timer.schedule(new TimerTask() { // from class: net.babycat.live.platform.widget.view.SnapUpCountDownTimerView.2
    ...
    public void stop() {
        ...
        if (timer != null) {
            timer.cancel();
            this.timer = null;
            b.b("充值奖励：倒计时销毁", new Object[0]);
        }
    }

    public SnapUpCountDownTimerView(Context context, @p0 AttributeSet attributeSet) {
